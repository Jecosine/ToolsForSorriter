...
# parse cmd argument
a.parse()
time_mat = np.loadtxt(os.path.join(dst_path, 'step5/dst.dat'), delimiter=' ', skiprows=1)
time_mat = np.concatenate((np.array([[0, ] + [float(i) / 4.0 for i in range(200)], ], dtype=np.float64), time_mat))
time_mat.transpose()
np.savetxt(os.path.join(dst_path, 'step4/dst.dat'), time_mat.transpose(), delimiter=' ')
